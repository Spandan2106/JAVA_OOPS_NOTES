...
{  
    ...
    MyFileFilter(String extension, String description)  
    {  
...
{  
    ...
    {  
        ...
        String family = (String)JOptionPane.showInputDialog(parent, "Choose a font:", title,  
                JOptionPane.PLAIN_MESSAGE, null, families, initialFont.getFamily());  
        if (family == null) return false;  
        
        String sizeText = JOptionPane.showInputDialog(parent, "Enter font size:", initialFont.getSize());  
        if (sizeText == null) return false;  
        
        try  
        {  
            int size = Integer.parseInt(sizeText);  
            selectedFont = new Font(family, initialFont.getStyle(), size);  
            return true;  
        }  
        catch (NumberFormatException ex)  
        {  
            selectedFont = new Font(family, initialFont.getStyle(), initialFont.getSize());  
            return false;  
        }  
    }  
    ...
    Font createFont()  
    {  
        return selectedFont != null ? selectedFont : initialFont;  
    }  
}  
...
{  
    ...
    void showDialog(Frame parent, boolean replaceMode)  
    {  
        String input = JOptionPane.showInputDialog(parent, replaceMode ? "Find text:" : "Find text:", findText);  
        if (input == null) return;  
        findText = input;  
        
        if (replaceMode)  
        {  
            String replacement = JOptionPane.showInputDialog(parent, "Replace with:", replaceText);  
            if (replacement != null)  
            {  
            ...
            if (index < 0) index = content.indexOf(findText, 0);  
            if (index >= 0)  
            {  
                textArea.setText(content.substring(0, index) + replaceText + content.substring(index + findText.length()));  
                textArea.setCaretPosition(index + replaceText.length());  
...
{  
    static void createLookAndFeelMenuItem(JMenu menu, Frame frame)  
    {  
        JMenuItem item = new JMenuItem("System Look & Feel");  
        ...
        {  
            ...
            {  
                ...
                {  
                    JOptionPane.showMessageDialog(frame, "Unable to change look and feel.", "Look and Feel", JOptionPane.ERROR_MESSAGE);  
                }  
            }  
        });  
        menu.add(item);  
